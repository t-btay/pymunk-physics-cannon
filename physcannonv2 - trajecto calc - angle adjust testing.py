# -*- coding: utf-8 -*-
"""
Created on Tue Aug 15 19:38:30 2023
                ...
                for r2 in range(cannon.xpos, tgt[0], 1):
                    ...
                    if pq: #this condiitonal block does the initial check for obstacles on the trajectory path and gives visual feedback on the drawn trajectory appropriately. continued obstacle checking for the modified trajectory will be done by the loop one level up.
                        ...
                        if pq.shape.body.body_type:
                            ...
                            pg.draw.circle(screen, "yellow", (r2, h2), 4)
                            pg.display.update()
    ...
    for event in pg.event.get():
        ...
        if event.type == pg.MOUSEBUTTONDOWN:
            ...
            if event.button == 1:
                ...
                elif bullet1.shape and bullet1.body:
                    try:
                        bullet.velocity = 0
                        bullet1.remove()
                        target_present = False
        ...
        if event.type == pg.KEYDOWN:
            ...
            if event.key == pg.K_p:
                ...
                pq = space.point_query_nearest(pg.mouse.get_pos(), 0, munk.ShapeFilter())
                if pq:
                    print(f"Point Query = {pq}")
                else:
                    print("Point Query = None")
